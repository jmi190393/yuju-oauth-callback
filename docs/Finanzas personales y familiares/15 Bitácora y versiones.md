...
| 2.0 | **Bot de WhatsApp** (arranque de Fase 2, ver [[16 Bot de WhatsApp]]): captura por texto (`150 uber`, etiquetas bebé/viaje/factura), `como voy`, `borrar`, `ayuda`. Webhook firmado (HMAC de Meta), solo números autorizados, dedupe de reintentos, cero dependencias nuevas. Reutiliza el categorizador con reglas aprendidas. 9 pruebas nuevas (25 unitarias en total + regresión). Módulo opcional: sin variables configuradas, la app funciona igual. |
| 1.9.2 | **Auditoría dedicada de `api.py`** (cuarta pasada): la ruta del asesor con IA ya no calcula todo dos veces (reutiliza presupuesto/suscripciones/metas de `_insights`); helper `_msi_pending()`; `/insights` deja de exponer claves internas; guard anti-carrera en el buscador de *Editar categorías*. Verificado: 16/16 unitarias + regresión real 25/25 con hash idéntico. |
| 1.9.1 | **Regresión con los 25 estados REALES** (recuperados de la sesión) convertida en prueba permanente (`tests/test_regression_pdfs.py`): 25/25 concilian, patrimonio $3,532,045 y MSI $136,602 exactos, **hash del parseo idéntico** antes/después. Con esa red se cerraron los refactors pospuestos de importadores: BBVA a una sola vía de extracción, mapa de meses único (`_util.MESES`), guardas en Revolut (fecha y CSV) y limpieza en Amex. |
| 1.9 | **Auditoría integral de arquitecto** (3 revisiones en paralelo). **Seguridad**: cookie de sesión con `Secure` en HTTPS; login de tiempo constante (evita enumerar usuarios). **Escalabilidad**: índices en `transactions.category_id`/`account_id` (con `CREATE INDEX IF NOT EXISTS` para la BD existente). **DRY**: helpers `_networth()` y `money()` (quita 16 repeticiones en los parsers). **Frontend**: buscador de Movimientos ya no pierde el foco al escribir + sin carreras de respuesta; `show()` captura errores (no más spinner colgado); `loading()`/`debounce()`; CSS muerto fuera. **Robustez**: Amex ya no aborta ante líneas ambiguas. **Nueva suite** `tests/test_importers.py`. Todo 16/16 en verde. Ver [[13 Arquitectura y calidad del código]]. |
...
- **Suite del Asesor** (`python3 tests/test_insights.py`, 13 casos, sin dependencias): casos borde (base vacía, ingreso $0 → sin divisiones por cero), agrupación de gastos hormiga, recurrentes solo con monto estable, alerta de cobro inusual, safe-to-spend, mes vs mes, proyección de metas, y el asesor con IA simulada (camino feliz) y sin llave (degradación).
- **Suite de importadores** (`python3 tests/test_importers.py`, 3 casos): equivalencia de `money()`, cobertura del mapa de meses de Amex, detectores de banco. Ancla los refactores de los parsers. Ambas corren en PythonAnywhere sin instalar nada.
